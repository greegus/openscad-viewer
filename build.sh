...
# The cache key has to change whenever the code that fills the manifest changes, or a cached
# manifest from an older build is served as current. Bumping a constant by hand failed twice —
# once for piece paths, once for hull outlines — so it is derived from the sources instead.
FINGERPRINT=$(cat Core/*.swift | grep -v "static let fingerprint" | shasum -a 256 | cut -c1-16)
cat > Core/Fingerprint.swift <<EOF
// Generated by build.sh — do not edit.
enum Build {
    /// Digest of Core/*.swift, so the render cache invalidates itself when they change.
...
swiftc -O -target "$TARGET" -sdk "$SDK" -module-name OpenSCADViewer \
  Core/ScadRenderer.swift Core/CSGSplitter.swift Core/Annotations.swift Core/Fingerprint.swift Core/Config.swift Core/GeometryProvider.swift \
...
swiftc -O -target "$TARGET" -sdk "$SDK" -module-name ScadRenderHelper \
  Core/ScadRenderer.swift Core/CSGSplitter.swift Core/Annotations.swift Core/Fingerprint.swift Core/Config.swift Core/GeometryProvider.swift QuickLook/RenderService.swift QuickLook/Helper/main.swift \
...
swiftc -O -target "$TARGET" -sdk "$SDK" -parse-as-library -module-name ScadThumbnail \
  Core/ScadRenderer.swift Core/CSGSplitter.swift Core/Annotations.swift Core/Fingerprint.swift Core/Config.swift Core/GeometryProvider.swift QuickLook/RenderService.swift QuickLook/XPCGeometryProvider.swift QuickLook/Thumbnail/ThumbnailProvider.swift \
