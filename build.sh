...
swiftc -O -target "$TARGET" -sdk "$SDK" -module-name ScadRenderHelper \
  Shared/ScadRenderer.swift Shared/RenderService.swift Shared/CSGSplitter.swift Helper/main.swift \
...
swiftc -O -target "$TARGET" -sdk "$SDK" -parse-as-library -module-name ScadThumbnail \
  Shared/ScadRenderer.swift Shared/RenderService.swift Shared/CSGSplitter.swift Thumbnail/ThumbnailProvider.swift \
...
swiftc -O -target "$TARGET" -sdk "$SDK" -parse-as-library -module-name ScadPreview \
  Shared/RenderService.swift Preview/PreviewViewController.swift Preview/WebGLProbe.swift Preview/ScadWebView.swift \
...
cp -R Web/ "$PREVIEW/Contents/Resources/Web/"
cp Preview/Info.plist "$PREVIEW/Contents/Info.plist"
cp App/Info.plist "$OUT/Contents/Info.plist"
cp Thumbnail/Info.plist "$APPEX/Contents/Info.plist"
printf 'APPL????' > "$OUT/Contents/PkgInfo"

echo "→ signing (ad-hoc)"
codesign --force --sign - --timestamp=none --entitlements Thumbnail/Thumbnail.entitlements "$APPEX"
codesign --force --sign - --timestamp=none --entitlements Thumbnail/Thumbnail.entitlements "$PREVIEW"
