...
enum ScadRenderer {
    ...
    /// Digest of the sources that decide what goes into a manifest, so a cached one from an
    /// older build is never served as current. This was a hand-bumped constant and was forgotten
    /// twice — pieces arrived without their tree paths once, and without hull outlines the next
    /// time — hence generated by build.sh rather than remembered.
    private static let format = Build.fingerprint
...
extension ScadRenderer {
    ...
    private static func exportMaterials(input: URL, cacheKey key: String) throws -> Data {
        ...
        func encode(_ b: CSGSplitter.Box) -> [String: Any] {
            var box: [String: Any] = ["matrix": b.matrix, "size": b.size, "centered": b.centered,
                                      "approximate": b.approximate]
            if let profile = b.profile { box["profile"] = profile }
            return box
        }
